...
```{r}
pf_depth_V <- PhyloFactor(OTU.matrix, Tree, X = metadata, frmla = Data~Depth_m, nfactors = 9, ncores=2, stop.early = T, choice='var')
```
...
```{r}
s_pf_depth.V.summary.1 <- summary(pf_depth_V, factor=1)
```

```{r}
pf.taxa(pf_depth_V, taxonomy, factor=1)$group1
```

Which, it turns out, encompasses bacteria in the Rhodobacteraceae group of the Alphaproteobacteria. This factor also appears to be tightly correlated to depth:

```{r}
ggplot(s_pf_depth.V.summary.1$data, aes(x = Depth_m, y = Data)) + 
geom_point() + 
geom_smooth(method='lm')
```

```{r}
s_pf_depth.V.summary.2 <- summary(pf_depth_V, factor=2)

pf.taxa(pf_depth_V, taxonomy, factor=2)$group1
```

```{r}
ggplot(s_pf_depth.V.summary.2$data, aes(x = Depth_m, y = Data)) + 
geom_point() + 
geom_smooth(method='lm')
```
...
#### Testing for phylogenetic groups correlated with meatadata: linear relationships

#### Testing for phylogenetic groups correlated with metadata: non-linear models

#### Testing for phylogenetic groups correlated with metadata: additive models